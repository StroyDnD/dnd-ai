  ...
  const handleGenerateLocationMap = async (locationName: string) => {
    ...
    try {
      ...
      if (result?.illustrations?.[0]) {
        ...
        // Add the new map to state with both imageBase64 (for immediate display) and publicUrl (for persistence)
        setLocationMaps((prev) => [...prev, { 
          locationName, 
          imageBase64, 
          publicUrl 
        }]);
      ...
      try {
        ...
        if (data && data.length > 0) {
          const formattedMaps = data.map(map => ({
            ...
            publicUrl: map.map_image_url
          }));
          
          setLocationMaps(formattedMaps);
        }
      } catch (error) {
        console.error("Error in fetchLocationMaps:", error);
      }
    };
    
    fetchLocationMaps();
  }, [params.id, user?.id, campaign]);
        ...
        <div className="bg-white/40 backdrop-blur-sm rounded-lg p-6">
          <div className="mb-8 flex justify-between items-center">
            <button
              ...
              className="cursor-pointer flex items-center font-cormorant hover:opacity-80 transition-colors"
            >
              <svg
