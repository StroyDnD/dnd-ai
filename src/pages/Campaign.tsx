  ...
  const handleGenerateLocationMap = async (locationName: string) => {
    ...
    try {
      ...
      if (result?.illustrations?.[0]) {
        ...
        const { data: locationMapRecord, error: locationMapError } = await supabase
          .from("location_maps")
          .insert({
        ...
        if (locationMapError) {
          console.error("Error inserting location map record:", locationMapError);
        ...
        await supabase.storage
          .from("campaign-maps")
          .upload(`${user?.id}/${campaign.id}/location-maps/${imageFile.name}`, imageFile, {
            cacheControl: "3600",
            upsert: true,
          });
        ...
        const {
          ...
          .from("campaign-maps")
          .getPublicUrl(`${user?.id}/${campaign.id}/locations-maps/${imageFile.name}`);
